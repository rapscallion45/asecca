...
import { getItinerary, setItinerary } from '@/lib/api';

/**
 * Proxy for handling requests to ASECCA Collection Form Itinerary API.
 *
  ...
  switch (method) {
    case 'POST':
      ...
      try {
        ...
        const response = await setItinerary(body);
    ...
    case 'GET':
      ...
      try {
        /* try proxying request to ASECCA API */
        const response = await getItinerary(query.collection);
