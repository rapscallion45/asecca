...
import { getQuote, setQuote } from '@/lib/api';

/**
 * Proxy for handling requests to ASECCA Collection Form Quote API.
 *
 ...
 * @param {NextApiResponse<ICollectionFormQuoteDataPayload | IProxyErrorPayload | null>} res - the response data
 * @returns {NextApiResponse} - handler response
 */
const handler = async (
  req: NextApiRequest,
  res: NextApiResponse<
    ICollectionFormQuoteDataPayload | IProxyErrorPayload | null
  >
) => {
  ...
  switch (method) {
    case 'POST':
      ...
      try {
        ...
        const response = await setQuote(body);
    ...
    case 'GET':
      ...
      try {
        /* try proxying request to ASECCA API */
        const response = await getQuote(query.collection);
